...
from action0.client.backends.urllib3 import _merged_headers
from action0.client.backends.urllib3 import _request_data
...
class Urllib3BackendTestCase(unittest.TestCase):
    """
    tests for :py:class:`action0.client.backends.urllib3.Urllib3Backend`
    ...
    def test_streamed_response_body(self) -> None:
        ...
        """
        with Urllib3Backend(stream=True) as backend:
    ...
    def test_redirects_followed_by_default_and_disablable(self) -> None:
        """
        Test both follow_redirects settings.
        """
        with Urllib3Backend() as backend:
            followed = backend.send(Request(f"{self.base_url}/redirect"))
        body = followed.body_str()
        assert body is not None
        self.assertEqual(json.loads(body)["path"], "/target")

        with Urllib3Backend(follow_redirects=False) as backend:
            stopped = backend.send(Request(f"{self.base_url}/redirect"))
    ...
    def test_timeout_is_translated(self) -> None:
        ...
        with retries disabled (bare timeout) and with urllib3's default
        retries (timeout wrapped in MaxRetryError).
        """
        with Urllib3Backend(timeout=0.05, retries=False) as backend:
            with self.assertRaises(TimeoutError) as caught:
                backend.send(Request(f"{self.base_url}/slow"))
        self.assertIsNotNone(caught.exception.request)

        with Urllib3Backend(timeout=0.05, retries=urllib3.Retry(1)) as backend:
    ...
    def test_connection_error_is_translated(self) -> None:
        ...
        with Urllib3Backend(timeout=1, retries=False) as backend:
            with self.assertRaises(TransportError):
                backend.send(Request(f"http://127.0.0.1:{port}/"))

    def test_close_semantics(self) -> None:
        """
        Test that an owned pool is emptied and a passed one is not.
        """
        own_pool = urllib3.PoolManager()
        backend = Urllib3Backend(own_pool)
        backend.send(Request(f"{self.base_url}/"))
        backend.close()
        self.assertGreaterEqual(len(own_pool.pools), 1)
        own_pool.clear()

        owned = Urllib3Backend()
        owned.send(Request(f"{self.base_url}/"))
        pool = owned._pool
        self.assertGreaterEqual(len(pool.pools), 1)
        owned.close()
...
class ConversionTestCase(unittest.TestCase):
    ...
    def test_merged_headers_joins_repeated_lines(self) -> None:
        """
        Test the RFC 9110 comma-merge for the mapping urllib3 needs.
        """
