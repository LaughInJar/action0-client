...
- `caching.py` — explicit TTL response caching as wrappers (deliberately NOT RFC 9111 — no Cache-Control/validators): `CachePolicy` (ttl, cacheable methods {GET, HEAD} and statuses {200}, `vary_headers` folded into the sha256 key; streaming bodies never stored), the two-method `CacheStore` protocol, `MemoryCache` (thread-safe LRU + per-entry expiry, injectable clock) and `CachingSyncBackend`/`CachingAsyncBackend`/`CachingDeferredBackend`. Hits are independent copies tied to the current request; on a hit the inner backend (and its hooks) is not involved.
