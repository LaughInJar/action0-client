...
- `backend.py` — the heart of the design. Three structural `Protocol`s, one per execution model: `SyncBackend` (`send() -> Response`), `AsyncBackend` (`send() -> Awaitable[Response]`), `DeferredBackend` (`send() -> Deferred[Response]`), plus the `Backend` union alias. Each protocol also has `map(result, fn)` — apply a function *inside* the backend's wrapper (plain call / await / `addCallback`). `map` is what lets `APIClient` attach response parsing without higher-kinded types, and its parameter types are what keep the three protocols structurally disjoint (a Deferred backend would otherwise also match `AsyncBackend`, since `Deferred` is awaitable). Base classes `BaseSyncBackend`/`BaseAsyncBackend`/`BaseDeferredBackend` implement `send` as a template around an abstract `_send` (raw I/O only) adding `Hook` instrumentation and `translate_error`. `BaseDeferredBackend` imports twisted only lazily inside `send`, so the module (and the stub subclass) is importable without twisted.
- `client.py` — `Client`, generic over the backend via the covariant `BackendT_co` (bound to the protocol union). The typing trick used throughout: `send` is `@overload`ed **on the self type** (`self: Client[SyncBackend] -> Response`, `self: Client[DeferredBackend] -> Deferred[Response]`, `self: Client[AsyncBackend] -> Awaitable[Response]`; Deferred before Async on purpose). Covariance makes `Client[RequestsBackend]` assignable to `Client[SyncBackend]`, which resolves the overload. The implementation body casts the union to one protocol pro forma (all three share the call shape).
...
- `api.py` — `APIClient`: backend + `base_url` + default `headers`; `send(operation)` overloaded on self type like `Client.send` but returning `R` / `Awaitable[R]` / `Deferred[R]`; pipeline is `prepare(operation.as_request(base_url))` → `backend.map(backend.send(request), operation.parse)`. `prepare()` merges default headers (gaps only) and is the override point for signing/auth.
...
- Tests mirror the `src/` layout under `tests/action0/client/` and are `unittest.TestCase` classes, executed via pytest. `tests/action0/client/test_typing.py` pins the static return-type promises with `typing.assert_type` — its `check_*` functions are analyzed by all three checkers but never executed.
