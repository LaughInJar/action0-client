...
- `operation.py` — `Operation[R_co]`: `@dataclass_transform` base class; `__init_subclass__` applies `dataclasses.dataclass(kw_only=True)` and validates (reserved names; path placeholders ↔ `path_param()` fields; at most one body form). ClassVars `method`, `path` (template with `{name}` placeholders), `accept`, `default_location` (placement of specifier-less fields, default QUERY). `as_request(base_url)` renders the path template, serializes fields per location (`None` = omitted; enums → value, dates → ISO, bools → true/false, lists → repeated params) and assembles the JSON or form-urlencoded body (+ Content-Type if unset). `parse()` = `check()` (2xx or `APIError`) + abstract `load()`. `JsonOperation[R_co]` implements `load` via JSON decode + overridable `load_json` (default returns payload as-is, for `JsonOperation[Any]`). No `typing.get_type_hints` anywhere — field placement lives in `dataclasses.field(metadata=...)`, so user modules may freely use `from __future__ import annotations`.
- `fields.py` — the PEP 681 field specifiers `query()`, `header()`, `path_param()`, `json_field()`, `json_body()`, `form_field()`, `body()` producing `dataclasses.field(metadata={"action0-client": FieldSpec(...)})`. The wire-name parameter is called `name`, NOT `alias` — PEP 681 reserves `alias` for renaming the `__init__` parameter and ty actually implements that. `repr=False` keeps secrets out of operation reprs.
- `api.py` — `APIClient[BackendT_co]`: backend + `base_url` + default `headers`; pipeline is `prepare(operation.as_request(base_url))` → `backend.map(backend.send(request), operation.parse)` — runtime-agnostic. The typing facade cannot be: "the backend's wrapper around R" needs higher-kinded types, so `send` is `@overload`ed **on the self type** for the shipped models (`self: APIClient[Backend[Response]] -> R`, `...[Backend[Deferred[Response]]] -> Deferred[R]`, `...[Backend[Awaitable[Response]]] -> Awaitable[R]`, `...[Backend[Future[Response]]] -> Future[R]` — Deferred before Awaitable is load-bearing, a Deferred *is* awaitable) plus a final catch-all overload returning `Any`, so backends with other wrappers are usable, just untyped — the documented escape hatch for precise typing of a custom wrapper is subclassing `APIClient` and re-declaring `send()` (one cast + one pyright override suppression; see the guide's "Other execution models" and `BoxAPIClient` in `test_typing.py`). Covariance of `BackendT_co` makes `APIClient[RequestsBackend]` assignable to `APIClient[Backend[Response]]`, which resolves the overloads; `client.backend` keeps the concrete backend type. `prepare()` merges default headers (gaps only) and is the override point for signing/auth.
...
- `retry.py` — backend-agnostic retries as wrappers (retrying is send-again, so it cannot be a `Hook`): `RetryPolicy` (frozen dataclass: attempts/backoff/multiplier/max_backoff, transient statuses, `retry_errors=(TransportError,)`, idempotent-methods gate — `methods=None` allows all; full jitter by default with injectable `rng`, `Retry-After` honored — seconds and HTTP-date — capped at max_backoff) + `RetryingSyncBackend`/`RetryingAsyncBackend`/`RetryingDeferredBackend`, each preserving the inner backend's wrapper type so the client typing is untouched. Exhausted attempts return the last response / raise the last error as-is. Sync/async take injectable `sleep` (async default `asyncio.sleep`; pass `trio.sleep` on trio), deferred takes a `reactor` (lazy global default) and waits via `deferLater`.
- `caching.py` — explicit TTL response caching as wrappers (deliberately NOT RFC 9111 — no Cache-Control/validators): `CachePolicy` (ttl, cacheable methods {GET, HEAD} and statuses {200}, `vary_headers` folded into the sha256 key; streaming bodies never stored), the two-method `CacheStore` protocol (plus its awaitable twin `AsyncCacheStore`, accepted by the async wrapper only — sync store calls run inline, async ones are awaited via `inspect.isawaitable`), `MemoryCache` (thread-safe LRU + per-entry expiry, injectable clock) and `CachingSyncBackend`/`CachingAsyncBackend`/`CachingDeferredBackend`. Hits are independent copies tied to the current request; on a hit the inner backend (and its hooks) is not involved.
- `backends/` — one module per HTTP stack: `requests.py` (`RequestsBackend`; multi-value response headers via urllib3 raw headers, producer bodies as chunk iterators), `httpx.py` (`HttpxBackend`/`AsyncHttpxBackend`; header lines and `multi_items()` keep multi-value fidelity; producer bodies stream sync/async), `aiohttp.py` (`AiohttpBackend`; the owned `ClientSession` is created lazily on first send — it must exist inside a running loop; timeouts arrive as `asyncio.TimeoutError`, checked before `ClientError`), `urllib3.py` (`Urllib3Backend` over `PoolManager.urlopen`; `retries=` passed through, timeouts unwrapped from `MaxRetryError`), `urllib.py` (`UrllibBackend`, stdlib-only; urllib's `HTTPError` for non-2xx is converted back into a `Response`; `_NoRedirects` handler for `follow_redirects=False`), `futures.py` (`ThreadPoolBackend`, stdlib-only; wraps any sync backend, `send` submits to a `ThreadPoolExecutor`, `map` chains via `add_done_callback` without occupying a worker; hooks belong on the wrapped backend), `twisted.py` (`TwistedBackend` over `Agent`/`RedirectAgent`, `readBody`, `_RequestBodyProducer` via `cooperate`, timeout via `addTimeout`; the global reactor is imported lazily in `__init__`, never at module import). All accept their library's native client/session/agent/pool (then never closed by the backend) or create their own (then closed by `close()`/`aclose()`).
...
- Tests mirror the `src/` layout under `tests/action0/client/` and are `unittest.TestCase` classes, executed via pytest. `tests/action0/client/test_typing.py` pins the static return-type promises with `typing.assert_type` — its `check_*` functions are analyzed by all three checkers but never executed. Its `Box`/`BoxBackend` (a wrapper type of its own) is the openness regression test: `Client.send` must type as `Box[Response]`, `APIClient.send` as `Any`, and `BoxAPIClient` pins the subclassing escape hatch.
...
- Docs live in `docs/` (Sphinx + Furo, MyST Markdown pages, autodoc for the API reference). Docstrings are Sphinx-reST (`:param:`, `:py:meth:` roles). CI builds them with `-W` on every run and deploys to GitHub Pages on pushes to `main`. Guide examples in `docs/usage/` (one page per topic, stitched together by `docs/usage/index.md`) show exact outputs in `#` comments — keep them truthful. `examples/petstore.py` is the complete worked example (type-checked in CI, runnable without network).
